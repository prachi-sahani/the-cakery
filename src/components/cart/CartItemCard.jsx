...
import { addToWishlist, removeFromCart, updateCart } from "../../utilities/server-request/server-request";
...
export function CartItemCard({item, calculatePriceDetails}){
    ...
    async function moveToWishlist(){
      ...
      const [updatedCart, updatedWishlist] = await Promise.all([removeFromCart(authToken, item._id),addToWishlist(authToken, item)]);
          ...
          <div className="card-container">
            ...
            <div className="card-content">
              <div className="product-price-description">
                <div className="product-price product-discount-price">
                  <i className="material-icons">currency_rupee</i>{item.price - item.discount}
                </div>
                ...
                <div className="product-price product-discount">
                  <i className="material-icons">currency_rupee</i>{item.discount} off
                </div>
              </div>
              <div className="qty-changer pt-2">
                ...
                <button className="btn-icon material-icons qty-increase" onClick={() => updateItem("increment")}>
                  add_circle_outline
                </button>
              </div>
            </div>
            <div className="card-footer">
              <div className="action-button">
                <button className="btn-basic btn-outline-primary btn-sm" onClick={moveToWishlist}>
                  Move to wishlist
                </button>
                <button className="btn-basic btn-outline-basic btn-sm" onClick={removeItem}>
                  Remove
                </button>
              </div>
            </div>
